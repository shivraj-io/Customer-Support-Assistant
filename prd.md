...
| 5 | Ticket list/history | In-memory list of processed tickets this session, newest first |
...
- Persistent database (in-memory store is fine; localStorage optional)
...
- Runs locally with one `npm install` + one command per service.
...
- FR5: Processed tickets persist in-memory on the backend for the session and are listed in a sidebar/history panel.
...
1. Backend endpoint that classifies + prioritizes + generates response (hardcode a test ticket, confirm JSON shape).
2. Frontend form → calls backend → renders result card.
3. History list.
4. Add sentiment, confidence, and next-action fields to the same LLM call (near-zero extra time — same prompt, more fields).
5. Seed ticket data + README (demo readiness).
6. Styling polish.
7. (Cut first if short on time) Edit-response feature, empty/error states, confidence-flag styling.
